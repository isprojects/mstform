...
test("decimal converter postprocessRaw no decimals before", async () => {
  const converter = converters.decimal({ decimalPlaces: 2 });
  const options = {
    decimalSeparator: ",",
    thousandSeparator: ".",
    renderThousands: true
  };
  const value = "4.314.314";
  const processedValue = converter.preprocessRaw(value, options);
  const converted = await converter.convert(processedValue, options);
  const rendered = await converter.render(
    (converted as ConversionValue<any>).value,
  ...
  expect(rendered).toEqual("4.314.314");
  const postprocessed = await converter.postprocessRaw(rendered, options);
  expect(postprocessed).toEqual("4.314.314,00");
});

test("decimal converter postprocessRaw one decimal before", async () => {
  const converter = converters.decimal({ decimalPlaces: 2 });
  const options = {
    decimalSeparator: ",",
    thousandSeparator: ".",
  ...
  expect(rendered).toEqual("4.314.314,0");
  const postprocessed = await converter.postprocessRaw(rendered, options);
  expect(postprocessed).toEqual("4.314.314,00");
});

test("decimal converter postprocessRaw decimalPlaces 0", async () => {
  const converter = converters.decimal({ decimalPlaces: 0 });
  const options = {
    decimalSeparator: ",",
    thousandSeparator: ".",
    renderThousands: true
  ...
  expect(rendered).toEqual("4.314.314");
  const postprocessed = await converter.postprocessRaw(rendered, options);
  expect(postprocessed).toEqual("4.314.314");
});
